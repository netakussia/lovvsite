...
/**
 * Скрипт миграции данных из старой версии в мультитенантную платформу
 * Использование: node migrate-existing-data.js
 */
...
// Пути к базам данных
const OLD_DB_PATH = './database-backup.sqlite';
const NEW_DB_PATH = './database.sqlite';
...
if (!fs.existsSync(OLD_DB_PATH)) {
    console.error('❌ Резервная копия базы данных не найдена:', OLD_DB_PATH);
    process.exit(1);
}
                                ...
                                oldDb.all("SELECT * FROM music_settings", (err, musicSettings) => {
                                    ...
                                    oldDb.all("SELECT * FROM temporary_messages", (err, tempMessages) => {
                                        ...
                                        oldDb.all("SELECT * FROM chat_messages", (err, chatMessages) => {
                                            ...
                                            console.log('\n📁 Резервная копия сохранена: database-backup.sqlite');
                                            console.log('\n🚀 Теперь вы можете:');
                                            ...
                                            console.log('   3. Управлять своим сайтом через админ-панель');
                                            console.log('   4. Просматривать сайт по адресу http://localhost:3000/site/migrated-love-site');
                                        });
                                    });
                                });
                            });
